function res = fitVirusCV19cs(getData,varargin)
...
%   20/04/16  CS change graph x axis to datetime values
%
% Usage:
...
%   own discretion.
    ...
    date0 = datetime(date0, 'ConvertFrom', 'datenum');
        date00 = date0;
        nday0 = length(C);
    % set end day
    if ~isempty(day)
    ...
    %... datums
    tp1 = real(tm - tau1) + date0;  % begin acceleration
    tp2 = real(tm) + date0;         % turning point
    tp3 = real(tm + tau2) + date0;  % end deceleration
   % tp4 = (tm + tau2) + tau + date0; % enter final phase
    tp4 = 2*tm + date0; % enter final phase

    %... dense forcast curve
    dt = 0.1;
    ttm = max(2.5*tm,datenum(date)-datenum(date0)+1);  % 20/04/02
    tspan = 0:dt:ttm;
    warning('on')
    ...
    if nargout > 0
        ...
        res.tp1   = datestr(floor(datenum(tp1)));
        res.tp2   = datestr(fix(datenum(tp2)));
        res.tp3   = datestr(ceil(datenum(tp3)));
        res.tp4   = datestr(ceil(datenum(tp4)));
        res.K     = K;
        res.r     = r;
        res.A     = K/I0 - 1;
        res.t2    = t2;
    ...
    if plt
        ...
        tx1 = sprintf('%s : coronavirus epidemic (SIR model): %s',...
            upper(country),datestr(date0 + length(tt) - 1));
        ...
        if nsp == 3
            ...
            dCe = diff(Ce);
             dC100 = (dCe./Ce(1:end-1))*100;
            
            %... plot prediction
            plot(tt(1:end-1)+date0,dC100,'LineWidth',2);
            
                      %... plot limits
            h = plot(tt(1:end-1)+date0,(5*ones(length(Ce)-1,1)),...
                'r','LineWidth',2);
            h.Annotation.LegendInformation.IconDisplayStyle = 'off';
            text(t(1)+date0,5,'5%','VerticalAlignment','bottom');
            
            if min(dC100) <= 1
                h = plot(tt(1:end-1)+date0,(ones(length(Ce)-1,1)),...
                    'g','LineWidth',2);
                h.Annotation.LegendInformation.IconDisplayStyle = 'off';
                text(t(1)+date0,1,'1%','VerticalAlignment','bottom');
            end
