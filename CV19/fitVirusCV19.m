    ...
    if plt
       ...
       % xlim([t(1),t(end)]+date0);
        ...
        plot(t(1:end-1) + date0,diff(Ca)/dt,'LineWidth',2)
         ...
         %... plot +/-SDE
        nsd = 1;
        h = plot(t(1:end-1) + date0,(diff(Ca)/dt + nsd*dRMSE),'r','LineWidth',1);
        h.Annotation.LegendInformation.IconDisplayStyle = 'off';
        Ct = (diff(Ca)/dt - nsd*dRMSE);
        Ct(Ct<0) = 0;
        h = plot(t(1:end-1) + date0,Ct,'r','LineWidth',1);
        h.Annotation.LegendInformation.IconDisplayStyle = 'off';
        ...
        if nsp == 3
            ...
            %... plot prediction
            plot(tt1(1:end-1)+date0,dC100,'LineWidth',2);
            
            %... plot limits
            h = plot(tt1(1:end-1)+date0,(5*ones(length(Ce1)-1,1)),...
                'r','LineWidth',2);
            h.Annotation.LegendInformation.IconDisplayStyle = 'off';
            npos = max([1,fix(0.8*length(tt1))]);
            text(tt1(npos)+date0,5,'5%','VerticalAlignment','bottom');
            
            if min(dC100) <= 1
                h = plot(tt1(1:end-1)+date0,(ones(length(Ce1)-1,1)),...
                    'g','LineWidth',2);
                h.Annotation.LegendInformation.IconDisplayStyle = 'off';
                text(tt1(npos)+date0,1,'1%','VerticalAlignment','bottom');
            end
